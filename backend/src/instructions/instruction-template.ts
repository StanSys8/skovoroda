/**
 * Instruction template served at GET /api/instructions/template.
 * Kept in sync with agent/instructions/template.md in the repo.
 * Section headings in English — the agent protocol language; the body
 * may be written in any language.
 */
export const INSTRUCTION_TEMPLATE = `# <Routine name>
...
export const REQUIRED_SECTIONS = ['Goal', 'Steps', 'Report'] as const;

/** Names of required sections missing from the given markdown. */
export function missingSections(content: string): string[] {
  return REQUIRED_SECTIONS.filter(
    (s) => !new RegExp(`^##\\s+${s}\\b`, 'im').test(content),
  );
}
